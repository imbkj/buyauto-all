(function($, w){
	...
	$(function(){
	    ...
	    table =$('#audit_list').dataTable({
	            	...
	            	ajax:{
	            		url: g_requestContextPath+'/h/l/audit/supplierList',
			            type: 'get',
			            async: false,
			            dataType: 'json',
			            dataSrc: 'rows',
	            	...
	            	columns:[
						...
						{data: function(d){
							...
							/*自营,审核不通过不能设置自营状态*/
							if(statusStr!=2){
								var selfSupport = d.selfSupport;
								if(selfSupport==null || selfSupport == "" || selfSupport==0){
									s += '<div class="table-o-tooltip">';
									s += '<a href="javascript:support(\''+d.id+'\');" class="btn-link">自营</a>';
									s += '</div>';
								}else{
									s += '<div class="table-o-tooltip">';
									s += '<a href="javascript:cancelSupport(\''+d.id+'\');" class="btn-link">取消自营</a>';
									s += '</div>';
								}
							}
							return s;
	        ...
	        //取消自营
	        w.cancelSupport = function(id){
	        	$.confirm({
				    title: '提示',
				    content: '确认取消该供销商自营状态?',
				    buttons: {
			        	confirm: {
				            text: '确认',
				            action: function(){
	       ...
	       /*修改自营状态*/
	       w.operSupport = function(id,status){
	    	   $.ajax({
	        		url:g_requestContextPath+"/h/l/audit/support",
	        		data:{"id":id,"support":status},
	        		type:"post",
	        		success:function(data){
	        			if(data == "200"){
	        				new PNotify({
	      ...
	      //确认冻结
	        function frozenCommpany(id,status){
	        	$.ajax({
	        		url:g_requestContextPath+"/h/l/audit/frozenSupplier",
