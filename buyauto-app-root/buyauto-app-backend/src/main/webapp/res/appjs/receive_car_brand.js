...
(function($, w) {
	...
	$(function() {
		...
		upload = new imageUploads($("#scremark"),false);//初始化file表单框//首页
	...
	w.tableDataLoad = function() {
	 ...
	 table = $('#transfer_datatable').dataTable({
		...
		serverSide : true,
		ajax : {
			url : g_requestContextPath+ '/h/l/configure/carBrandList',
			type : 'post',
			async : false,
			dataType : 'json',
			dataSrc : 'rows',
		...
		$("#btn_submit").click(function() {
			...
			if(upload.getNewFile().length == 0){
				if(upload.getOldFile() ==null){
					...
					$("#scremarkH").val(upload.getOldFile()[0].name);
				}else{
					$("#scremarkH").val();
				}	
			}else{
				$("#scremarkH").val(upload.getNewFile()[0].name);
			}

			//console.log(upload.getNewFile());
			//console.log(upload.getOldFile());
			//return false;
			...
			if($("#scremarkH").val() == "" || $("#scremarkH").val() ==null || $("#scremarkH").val() =="[]"){
				new PNotify({
					title : '提示消息',
					text : '品牌图标不能为空',
					type : 'error',
			...
			$.ajax({
				url : g_requestContextPath + "/h/l/configure/addCarBrand",
				type : 'post',
				dataType : 'json',
				data : {
					"id" : $('#id').val(),
					"scValue" : $('#scvalue').val(),
					"scRemark":$('#scremarkH').val()
...
w.showUpdateAddress= function(id){
		...
		$.get(g_requestContextPath + "/h/l/configure/updataShowAddress", {
			...
			if(data){
				var uploadFigure = new Array();
				
				if(data.scRemark){
					uploadFigure.push({filePath:data.imgfilePath,name:data.scRemark});
				...
				//uploadFigure.push({filePath:data.imgfilePath,name:data.scRemark,preId:""})
				$("#myUASModalLabel").html("品牌修改");
				$("#editwin").modal('show');
			}
