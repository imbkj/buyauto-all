...
(function($, w) {
	...
	w.tableDataLoad = function() {
	 ...
	 table = $('#transfer_datatable').dataTable({
		...
		serverSide : true,
		ajax : {
			url : g_requestContextPath+ '/h/l/configure/optionalList',
			type : 'post',
			async : false,
			dataType : 'json',
			dataSrc : 'rows',
	...
	return table;
		...
		$("#btn_submit").click(function() {
			...
			$.ajax({
				url : g_requestContextPath + "/h/l/configure/addOptional",
				type : 'post',
				dataType : 'json',
				data : {
					"id" : $('#id').val(),
...
w.showUpdateAddress= function(id){
		removeList();
		$.get(g_requestContextPath + "/h/l/configure/updataShowOptional", {
			"id" : id
		},function(data) {
