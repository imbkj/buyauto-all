...
$(function(){
	...
	upload02.init("image");
	upload03 = new imageUploads($("#imageInput03"),true);//初始化file表单框//首页
	upload03.init("pdf");
	...
	$('#proBtn03').click(function(){
		if(isok){
	        ...
	        if(fileObjLen<1){
	     	   hs = 1;
	        	$('.Errors').show();
        		$('.Errors').find('span').html('凭证上传不能为空')
	        	return;
	        }
	        if(fileObjLen > 3){
	        	hs = 1;
...
function getInfo(){
	//console.log(pageNumber)
	$.ajax({
		url: g_requestContextPath + "/h/l/supplier/getMyOrders",
		type: "post",
		data:{
			"page":pageNumber,
			"rows":pageSize,
		...
		success:function(data){
			...
			for(var i=0;i<data.rows.length;i++){
				...
				if(data.rows[i].status == 106){
					carIndent+=		'<span class="take_car certificate" id="'+ data.rows[i].status +'" data-id="'+data.rows[i].id+'">上传三证</span>';
				}
					...
					uploadDepositFigure=new Array();
			 		$.ajax({
		       			...
		       			success:function(data){
		       				...
		       				upload03.init("pdf",uploadDepositFigure);
		       			}
		       		});
			 		$('.boom').show();
			 		$('.pro03').show();
