...
//小图切换过程
	function minshow(xtqhnum){
		...
		var mingdjl_w=-mingdjl_num*picminw + picminw;
		...
		if(can.mindire==true){
			...
			if(picminnum>can.min_picnum){
				...
				if(xtqhnum==picminnum-1){mingdjl_w=-(mingdjl_num-1)*picminw+picminw;}
...
//大图切换过程
		function show(tpqhnum){
			...
			$(can.pnum).find('li a').eq(tpqhnum).addClass("on").parent().siblings().children(this).removeClass("on");
		};
//弹出框图片切换过程
